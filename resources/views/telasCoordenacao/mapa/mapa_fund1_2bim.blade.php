<html>
    ...
    <body>
        ...
        @empty
        @endforelse
        <div class="mapa-titulo"> MAPAS DE NOTAS - 2º BIMESTRE - {{$turma->AnoLetivo}}</div>
...
@forelse($Alunos as $Aluno)      
    ...
    <!-- pecorrendo a lista da disciplinas existente da turma e para consulta se existe nota-->
        ...
        @forelse($notasDoAluno as $nota)
                    <!-- Verifiva se existe noda de AB2 da X disciplina -->
                    @if (isset ($nota->AB2))
                                <!-- imprimindo a nota -->
                                @if (($nota->AB2) == 0)
                                    <td class="Mapa_valorX"><center>X</center></td>
                                @elseif (($nota->AB2) < 7)
                                <td><center><div class="Mapa_notaVermelha">{{number_format($nota->AB2 ,1)}}</div></center></td> 
                                @else
                                <td><center><div class="Mapa_notaAzul">{{number_format($nota->AB2 ,1)}}</div></center></td> 
                                <!-- / imprimindo a nota -->
                                @endif    
                    <!-- / Verifiva se existe noda de AB2 da X disciplina -->
                    @endif
