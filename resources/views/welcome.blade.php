...
@section('content')
    <div class="flex flex-col gap-8">
        ...
        @php
            $nomeUsuario = 'Nayana';
            if (auth()->guard('guardLogin')->check()) {
                $cpf = auth()->guard('guardLogin')->user()->CPFUsuario;
                $dadosFunc = \App\Models\modelCoordenacao\tb_usuario::infUsuario($cpf)->first();
                if ($dadosFunc && !empty($dadosFunc->NomeFuncionario)) {
                    $partes = explode(' ', trim($dadosFunc->NomeFuncionario));
                    $nomeUsuario = ucfirst(mb_strtolower($partes[0]));
                }
            }
        @endphp
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                <h1 class="text-3xl font-semibold text-[#0a241e]">Bem-vinda, {{ $nomeUsuario }}</h1>
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div class="bg-white border border-[#e3e8e6] rounded-2xl p-6 shadow-2xs hover:shadow-xs transition-shadow">
                <div class="text-3xl font-bold text-[#008a4b]">{{$quantAlunosCadastrados}}</div>
                <div class="text-sm font-semibold text-[#7fa398] mt-1">Alunos Cadastrados</div>
            </div>
            <div class="bg-white border border-[#e3e8e6] rounded-2xl p-6 shadow-2xs hover:shadow-xs transition-shadow">
                <div class="text-3xl font-bold text-[#008a4b]">{{$quantMatriculasAtivas}}</div>
                <div class="text-sm font-semibold text-[#7fa398] mt-1">Alunos Ativos</div>
            </div>
            <div class="bg-white border border-[#e3e8e6] rounded-2xl p-6 shadow-2xs hover:shadow-xs transition-shadow">
                <div class="text-3xl font-bold text-[#008a4b]">{{$quantMatriculasInativas}}</div>
                <div class="text-sm font-semibold text-[#7fa398] mt-1">Alunos Inativos</div>
            </div>
            <div class="bg-white border border-[#e3e8e6] rounded-2xl p-6 shadow-2xs hover:shadow-xs transition-shadow">
                <div class="text-3xl font-bold text-[#008a4b]">{{$quantUsuarioCadastrados}}</div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 flex flex-col gap-6">
                ...
                <!-- Componente de Gráficos de Indicadores Escolares com dados reais -->
                <x-dashboard-charts 
                    :matriculas="$chartMatriculas ?? []" 
                    :notasLabels="$chartNotasLabels ?? []" 
                    :notasValues="$chartNotasValues ?? []" 
                    :mensalidades="$chartMensalidades ?? []" 
                />
            </div>
            ...
            <div class="flex flex-col gap-6">
                <div class="bg-white border border-[#e3e8e6] rounded-2xl p-6 shadow-2xs hover:shadow-xs transition-shadow">
                    <div class="text-3xl font-bold text-[#008a4b]">{{$quantTurmasAtivas}}</div>
                    <div class="text-sm font-semibold text-[#7fa398] mt-1">Turmas Ativas</div>
                </div>
                <div class="bg-white border border-[#e3e8e6] rounded-2xl p-6 shadow-2xs hover:shadow-xs transition-shadow">
                    <div class="text-3xl font-bold text-[#008a4b]">{{$quantDisciplina}}</div>
                    <div class="text-sm font-semibold text-[#7fa398] mt-1">Disciplinas</div>
                </div>
                <div class="bg-white border border-[#e3e8e6] rounded-2xl p-6 shadow-2xs hover:shadow-xs transition-shadow">
                    <div class="text-3xl font-bold text-[#008a4b]">{{$quantFuncionariosCadastrados}}</div>
