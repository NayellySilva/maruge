...
<div class="flex flex-col gap-6">
    ...
    <div class="text-sm text-[#5c706b]">
        ...
        <span class="font-semibold text-[#0a241e]">Recibos / Carnês</span>
    </div>

    <!-- Cabeçalho Principal -->
    <div class="flex justify-between items-center">
        <div class="flex flex-col gap-1">
            <h1 class="text-3xl font-semibold text-[#0a241e]">Carnês de Mensalidades</h1>
    ...
    <div class="flex flex-col sm:flex-row gap-4 items-center">
        ...
        <div class="w-full sm:w-80">
            <form method="POST" action="{{ url('/coordenacao/carne_pesq') }}" class="w-full">
                @csrf
        ...
        <div class="w-full sm:w-64">
            <form method="GET" action="{{ url()->current() }}" class="w-full">
                <div class="relative" id="dropdown-container-turma-carne">
                    <input type="hidden" id="idTurmas" name="idTurmas" value="{{ request()->input('idTurmas', '') }}">

                    @php
                        $selectedTurmaCarne = $turmas->firstWhere('idTurmas', request()->input('idTurmas'));
                    @endphp

                    <!-- Trigger Box -->
                    <div onclick="toggleMultiDropdown('dropdown-menu-turma-carne', 'chevron-turma-carne')" 
                        ...
                        <span id="label-turma-carne" class="text-sm font-medium truncate {{ $selectedTurmaCarne ? 'text-[#0a241e]' : 'text-[#95aba5]' }}">
                            {{ $selectedTurmaCarne ? $selectedTurmaCarne->NomeTurma : 'Filtrar por Turma' }}
                        </span>
                        <div id="chevron-turma-carne" class="text-[#95aba5] transition-transform duration-200 shrink-0 ml-2">
                            <i data-lucide="chevron-down" class="w-4 h-4"></i>
                        </div>
                    </div>

                    <!-- Dropdown Flutuante -->
                    <div id="dropdown-menu-turma-carne" class="hidden absolute top-full left-0 right-0 mt-1 bg-white border border-[#e3e8e6] rounded-xl shadow-xl z-50 p-2 flex flex-col gap-2 overflow-hidden" style="max-height: 240px;">
                        <!-- Campo de Busca -->
                        <div class="relative shrink-0">
                            <input type="text" onkeyup="filterDropdownOptions('search-turma-carne', 'option-turma-carne')" id="search-turma-carne" placeholder="Pesquisar..." class="w-full pl-3 pr-9 py-1.5 bg-[#f8faf9] border border-[#e3e8e6] rounded-lg text-xs focus:outline-none focus:border-[#008a4b]">
                        ...
                        <div class="custom-scroll flex flex-col gap-0.5 pr-1" style="max-height: 180px; overflow-y: auto;">
                            <div onclick="selectSingleOption('', 'Todas as Turmas', 'idTurmas', 'label-turma-carne', 'dropdown-menu-turma-carne', 'chevron-turma-carne', true)"
                                 class="option-turma-carne flex items-center p-2 hover:bg-[#ecfdf5] rounded-lg transition-colors cursor-pointer text-xs text-[#0a241e]">
                                <span class="option-title font-medium">Todas as Turmas</span>
                            </div>
                            @foreach($turmas as $t)
                                <div onclick="selectSingleOption('{{ $t->idTurmas }}', '{{ $t->NomeTurma }}', 'idTurmas', 'label-turma-carne', 'dropdown-menu-turma-carne', 'chevron-turma-carne', true)"
                                     class="option-turma-carne flex items-center p-2 hover:bg-[#ecfdf5] rounded-lg transition-colors cursor-pointer text-xs text-[#0a241e]">
    ...
    <!-- Tabela de Alunos e Opções de Carnê -->
            ...
            <table class="w-full border-collapse">
                <thead>
                    <tr class="bg-[#f8faf9] border-b border-[#e3e8e6]">
                        ...
                        <th class="px-6 py-4 text-center text-xs font-semibold uppercase tracking-wider text-[#5c706b]">Ações / Carnê</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-[#e3e8e6]">
                    <!-- Laço exibindo os alunos retornados -->
                        ...
                        <tr class="hover:bg-[#f8faf9]/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-sm text-center">
                                <div class="flex justify-center gap-2">
                                    <!-- Criar / Configurar Carnê -->
                                    <a href="{{ url('/coordenacao/criarcarne/' . $Aluno->idAluno) }}"
                                       class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-emerald-50 text-emerald-700 hover:bg-emerald-100 transition-colors"
                                       title="Gerar Carnê">
                                        <i data-lucide="credit-card" class="w-3.5 h-3.5"></i> Gerar Carnê
                                    </a>

                                    <!-- Imprimir Carnê -->
                                    <a href="{{ url('/coordenacao/carner/' . $Aluno->idAluno) }}"
                                       target="_blank"
                                       class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-blue-50 text-blue-700 hover:bg-blue-100 transition-colors"
                                       title="Imprimir Carnê">
                                        <i data-lucide="printer" class="w-3.5 h-3.5"></i> Imprimir Carnê
                                    </a>
                                </div>
                            </td>
                        </tr>
