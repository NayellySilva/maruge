...
<div class="flex flex-col gap-6">
    ...
    <div class="text-sm text-[#5c706b]">
        ...
        <span class="font-semibold text-[#0a241e]">Novo Acordo</span>
    </div>

    <!-- Cabeçalho Principal -->
    <div class="flex justify-between items-center">
        <div class="flex flex-col gap-1">
            <h1 class="text-3xl font-semibold text-[#0a241e]">Acordos Financeiros</h1>
        ...
        <div class="w-full sm:w-64">
            <form method="GET" action="{{ url()->current() }}" class="w-full">
                <div class="relative" id="dropdown-container-turma-acordo">
                    ...
                    @php
                        $selectedTurmaAcordo = $turmas->firstWhere('idTurmas', request()->input('idTurmas'));
                    @endphp

                    <!-- Trigger Box -->
                    <div onclick="toggleMultiDropdown('dropdown-menu-turma-acordo', 'chevron-turma-acordo')" 
                        ...
                        <span id="label-turma-acordo" class="text-sm font-medium truncate {{ $selectedTurmaAcordo ? 'text-[#0a241e]' : 'text-[#95aba5]' }}">
                            {{ $selectedTurmaAcordo ? $selectedTurmaAcordo->NomeTurma : 'Filtrar por Turma' }}
                        </span>
                        <div id="chevron-turma-acordo" class="text-[#95aba5] transition-transform duration-200 shrink-0 ml-2">
                    ...
                    <div id="dropdown-menu-turma-acordo" class="hidden absolute top-full left-0 right-0 mt-1 bg-white border border-[#e3e8e6] rounded-xl shadow-xl z-50 p-2 flex flex-col gap-2 overflow-hidden" style="max-height: 240px;">
                        ...
                        <div class="custom-scroll flex flex-col gap-0.5 pr-1" style="max-height: 180px; overflow-y: auto;">
                            ...
                            @foreach($turmas as $t)
                                <div onclick="selectSingleOption('{{ $t->idTurmas }}', '{{ $t->NomeTurma }}', 'idTurmas', 'label-turma-acordo', 'dropdown-menu-turma-acordo', 'chevron-turma-acordo', true)"
                                     class="option-turma-acordo flex items-center p-2 hover:bg-[#ecfdf5] rounded-lg transition-colors cursor-pointer text-xs text-[#0a241e]">
    ...
    <!-- Tabela de Alunos e Geração de Acordo -->
            ...
            <table class="w-full border-collapse">
                <thead>
                    <tr class="bg-[#f8faf9] border-b border-[#e3e8e6]">
                        ...
                        <th class="px-6 py-4 text-center text-xs font-semibold uppercase tracking-wider text-[#5c706b]">Gerar Acordo</th>
                    </tr>
                </thead>
                    ...
                    @forelse($Alunos as $Aluno)
                        <tr class="hover:bg-[#f8faf9]/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-sm text-center">
                                <a href="{{ url('/coordenacao/criaracordo/' . $Aluno->idAluno) }}"
                                   ...
                                   title="Gerar Acordo">
                                    <i data-lucide="file-signature" class="w-3.5 h-3.5"></i> Gerar Acordo
                                </a>
                            </td>
                        </tr>
                    @empty
                        <!-- Estado exibido quando nenhum aluno e encontrado -->
                        <tr>
