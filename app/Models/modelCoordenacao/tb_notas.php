...
class tb_notas extends Model {
    ...
    public function RnotasAluno() {
     ...
     //Relacionamento com a tabela dsiciplina
    public function disciplinas() {
        return $this->hasOne('App\Models\modelCoordenacao\tb_disciplinas', 'idDisciplinas', 'tb_disciplinas_idDisciplinas');
    }
...
// Buscando a nota do aluno, referente a disciplina informada no parametro
    public static function busca_notas_do_aluno_grafico($idAluno) {
        return tb_notas::select()
               ->select('tb_notas.*')
               //->join('tb_disciplinas', 'tb_disciplinas.idDisciplina', '=', 'tb_notas.tb_disciplinas_idDisciplinas')
               ->where('tb_notas.tb_aluno_idAluno', '=', $idAluno)
                ->get();
    }
// Buscando a nota do aluno, referente a disciplina informada no parametro
    public static function busca_notas_do_aluno_grafico_bimestres($idAluno) {
       
        return tb_notas::select()
          
          //  ->whereRaw('tb_disciplinas.idDisciplina = tb_notas.tb_disciplinas_idDisciplinas') 
           ->join('tb_disciplinas', 'tb_disciplinas.idDisciplinas', '=', 'tb_notas.tb_disciplinas_idDisciplinas')
           ->where('tb_notas.tb_aluno_idAluno', '=', $idAluno)
                
                
          ->select('tb_notas.*','tb_disciplinas.NomeDisciplina')
...
//Editando nota atravez do seu id principal.
    public static function editandoNotas($dadosForm) {
        $idNotas = tb_notas:: select()->find($dadosForm["idNotas"]);
        ...
        if ($count > 0) { // SE CONTE FOR MAIOR QUE ZERO ELE ENTRA NO FOR
            ...
            for ($i = 0; $i < $count; $i++) {
                if (!empty($dadosForm["AB1"][$i])) {
                    ...
                    $contador = tb_notas::select()// buscando disciplina/turma e aluno para verificar se é caso de salvar ou atualizar pois cada disciplina tem seus bimestres
                            ->select('tb_notas.*')
                    ...
                    if ($contador >= 1) {// se a busca acima retorna verdadeiro, ou seja estive dados ele tem que pegar os dados e atualizar
                        ...
                        tb_notas::create($nota); // SALVANDO OS DADOS NO BANCO
                    }
                }
            }
            return "notalancada";
    ...
    public static function salva_nota_1bim_fun1($dadosForm) {
        ...
        if ($count > 0) { // SE CONTE FOR MAIOR QUE ZERO ELE ENTRA NO FOR
            $nota = [];   // CRIANDO UM ARRAY NOVO PRA FICA RECEBENDO DADOS DOS ARRAYS
            for ($i = 0; $i < $count; $i++) {
                if (isset($dadosForm["AB1"][$i]) || isset($dadosForm["RB1"][$i])) {
                    $nota = [// ARRAY RECEBENDO OS DADOS DO FOR
