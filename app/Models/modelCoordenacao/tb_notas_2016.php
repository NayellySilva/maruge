...
class tb_notas_2016 extends Model {

    protected $table = 'tb_notas_2016';
...
// Buscando a nota do aluno, referente a disciplina informada no parametro
    public static function busca_notas_do_aluno_2016($idAluno, $disciplina) {
        return tb_notas_2016::select()
                        ->select('tb_notas_2016.*')
                        ->where('tb_notas_2016.tb_aluno_idAluno', '=', $idAluno)
                        ->where('tb_notas_2016.tb_disciplinas_idDisciplinas', '=', $disciplina)
                        ->get('AB1');
    }
//Editando nota atravez do seu id principal.
    public static function editandoNotas($dadosForm) {
        $idNotas = tb_notas_2016:: select()->find($dadosForm["idNotas"]);
        ...
        if ($count > 0) { // SE CONTE FOR MAIOR QUE ZERO ELE ENTRA NO FOR
            ...
            for ($i = 0; $i < $count; $i++) {
                if (!empty($dadosForm["AB1"][$i])) {
                    ...
                    $contador = tb_notas_2016::select()// buscando disciplina/turma e aluno para verificar se é caso de salvar ou atualizar pois cada disciplina tem seus bimestres
                            ->select('tb_notas_2016.*')
                    ...
                    if ($contador >= 1) {// se a busca acima retorna verdadeiro, ou seja estive dados ele tem que pegar os dados e atualizar
                        $atualizando[] = tb_notas_2016::select()
                                ->select('tb_notas_2016.*')
                                ->where('tb_disciplinas_idDisciplinas', '=', $dadosForm["tb_disciplinas_idDisciplinas"][$i])
                                ->where('tb_turmas_idTurmas', '=', $dadosForm["tb_turmas_idTurmas"][$i])
                                ->where('tb_aluno_idAluno', '=', $dadosForm["tb_aluno_idAluno"][$i])
                        ...
                        tb_notas_2016::create($nota); // SALVANDO OS DADOS NO BANCO
                    }
                }
            }
            return "notalancada";
